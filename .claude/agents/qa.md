...
- **No matas procesos que no arrancaste tú, y nunca por patrón.** Nada de `pkill`, `killall` ni `kill` por nombre: esta máquina la comparten otras sesiones de agentes en otros proyectos, y un `pkill -f vite` ya se llevó por delante el servidor de desarrollo de otro repo. Lo que arrancas tú lo arrancas con **`set -m` en su propia línea** (dentro de un `bash -c` el control de trabajos viene apagado y el hijo hereda el grupo de la shell), guardas `DEV=$!`, y lo matas por su grupo comprobando antes que ese PID **es** su grupo: `[ "$(ps -o pgid= -p "$DEV" | tr -d ' ')" = "$DEV" ] && kill -TERM -"$DEV" || kill -TERM "$DEV"`. Tres trampas comprobadas en vivo: **(1)** `set -m && CMD &` **desarma la receta y el guardia da verde igual** — el `&` toma la lista `&&` entera, `$!` apunta a una subshell que es su propio líder de grupo, y el proceso real queda en otro grupo, así que se mata la subshell y el hijo sobrevive; **(2)** después de matar **hay que comprobar que murió** (`ss -ltnp | grep ':<puerto>'`), porque sin ese paso lo anterior es invisible; **(3)** `ps` **no dice de quién es** un proceso —un vite sale como `sh -c vite`, sin ruta—: el dueño se identifica por el puerto (`ss -ltnp`) y por `readlink /proc/<pid>/cwd`, y eso va antes de cualquier `kill`. Con `setsid` no funciona: bifurca y `$!` es el del `setsid` ya muerto. Un puerto ocupado por algo ajeno **no se libera**: se reporta como *no probado*.
